...
#--------------CSV Creation--------------
csvFile = open('ATWR_61850_Signals_2.csv', 'w')
csvFile.write("Report, Time Tag, Quality Tag, Signal Tag, Physical Device, DataType\n" )

#--------------XML Parsing--------------

DirectoryStr = 'ATWR_500.scd' #For BBTU file
...
#---------Logical Device---------------------
def getLogicalDevice( PDElem, inst ):           #Argument = Physical Device, Logical device
...
def getLogicalNode(LDElem):
    ...
    for LNElem in LDElem:
        ...
        if RCElems != []:     #If Logical Node contains Report, must have dataset
            #Only Logical Nodes with Report Control block will go through
...
##                ReportControlStr = PhysicalDeviceStr + LogicalDeviceStr
                ...
                if RCElem.get("buffered") is not None:
                    ...
                    ReportControlStr = PhysicalDeviceStr + LogicalDeviceStr + "/" + lnClassStr + "$" + "RP"+"$" + RCElem.get("name") + "0" + ReportNumberStr
                    #if buffering not specified, just assume it is a report??
                if DSElem != None:
...
def getDataSetInfo(DSElem, LDElem, LNTypeStr, RCElem):  #Take all parents, PD not included because it is already a global variable
    global LogicalDeviceStr
    ...
    for FCDAElem in DSElem:
        ...
        #Logical Device String - Turns out not all report control fcda points to the same logical device
        LogicalDeviceStr = ""
        LogicalDeviceStr = ldInst
        ...
        #-----------The if is to make sure no debug error when there is no attribute value in the FCDA Element
        ...
        if lnInst != None:   #LNType is defined in FCDA
            #FCDA Element's Logical node!! -> to get LNodeType from DataTypeTemplate
            ...
            getLNodeTypeElem(LNTypeStr2,doNameList, fc)     #Get from <DataTypeTemplate> Element (Same Level as PD Element)
        else:  #for Regda - LNType is defined in LNode
            getLNodeTypeElemR(doNameList, LNTypeStr,fc)     #Get from <DataTypeTemplate> Element (Same Level as PD Element)
...
##    LNElem = LDElem.find("x:LN/[@lnClass='"+lnClass+"'][@inst='"+lnInst+"']", namespaces)
    if prefix is None:  #Search for LN of DS FCDA without prefix (child elem of LD)
...
def getLNodeTypeElem(LNTypeStr,doNameList, fc):
    ...
    #-------------------------------190127_Addition for GRNE CMMXU-----------------------
    if DOTypeElem2.find("x:SDO",namespaces) is not None:
        doNameStr = doNameList[0]
        SDOElems = DOTypeElem2.findall("x:SDO", namespaces)
        for SDOElem in SDOElems:
            sdoNameStr = SDOElem.get("name")
            sdoTypeStr = SDOElem.get("type")

            DataObjectList = []
            DataObjectList.append(doNameStr)
            DataObjectList.append(sdoNameStr)

            DOTypeElem3 = root.find("x:DataTypeTemplates/x:DOType/[@id='"+sdoTypeStr+"']", namespaces)
            DAElems = DOTypeElem3.findall("x:DA/[@fc='"+fc+"']", namespaces)
            #getDAElem(DAElems)
            for DA in DAElems:
                DataTypeList = []
                DataAttributeList = []
                ...
                DATypeStr = DAElem.get("bType")
                DataAttributeList.append(DAStr)
                DataTypeList.append(DATypeStr)

                if(DAElem.get('bType')=="Struct"):
                    DAType = DAElem.get('type')
                    DATypeElem2 = root.find("x:DataTypeTemplates/x:DAType/[@id='"+DAType+"']", namespaces)
                    #print "inside bType 1"
                    for BDAElem in DATypeElem2:
                        ...
                        if (BDAElem.get("bType")=="Struct"):
                            ...
                            for BDAElem in DATypeElem3:
                                BDANameStr = BDAElem.get("name")
                                BDANTypeStr2 = BDAElem.get("bType")
                                DataAttributeList.append(BDANameStr)
                                DataTypeList.append(BDANTypeStr2)
                                TagPrinter()
                        else:
                            TagPrinter()
                else:
...
def TagPrinter():
    ...
    IECTagDict2 = {'ReportControlStr':'', 'PhysicalDeviceStr':'','LogicalDeviceStr':'','LogicalNodeStr':'','FunctionalConstraintsStr':'','DataObjectList':[],'DataAttributeList':[],'DataTypeList':[]}
...
def printTagCommand():
    ...
    for IECTag in IECTagDictList:
        ...
        #4. Create Signal Tags and FCDA Tag
            ...
            OutputBTypeStrList.append(IECTag['DataTypeList'][-1])
    ...
    for SigTag in SignalTagList:
        if FCDAEnable ==1:
            ...
            print ReportControlTag + "," + TimeTag + "," + QualityTag + "," + SigTag + "," + IECTag['PhysicalDeviceStr'] + "," + OutputBTypeStrList[i]
            csvFile.write(ReportControlTag + "," + TimeTag + "," + QualityTag + "," + SigTag + "," + IECTag['PhysicalDeviceStr'] + "," + OutputBTypeStrList[i] + "\n")
            i+=1
    del OutputBTypeStrList[:]
...
def main():
    ...
    for PDElem in root.findall("x:IED", namespaces):       #B01AF34C , B03F90AVR3,  [@name='B01AF34C']    D02AF34C
        ...
        #-----------------General-----------------
        RCAvaiLDList = []       #Report Control Available in Logical Device list
        ...
        if len(RCAvaiLDList) != 0:
            for element in RCAvaiLDList:
                LDElem = getLogicalDevice(PDElem, str(element))     #to get the logical device that has been filtered to contain report control blocks
